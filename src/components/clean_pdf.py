...
def my_cleaner(document, min_len_tokens, noisy_words = [], lemmatization=False, stemming=False, nouns = False, camel = True, removeNums = False, lower = False,\
               newLineRemove=True, punc = False, uni = True , whitespace = False, stopwords =False):
    '''
...
def clean_pdf(pdf):
    ...
    # df_raw['raw_paras'] = [my_cleaner(x,0,newLineRemove = False,lemmatization=False,nouns = False) for x in df_raw['raw_paras']]    # apply cleaner function to each row of dataframe
    # df = df_raw.copy()
